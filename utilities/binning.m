...
for Ib=1:1:Nbin,
   ...
   If = find( abs(ResMat(Ib,1)-Data(:,Col.X))<(0.5.*BinSize) );
   ResMat(Ib,2) = mean( Data(If,Col.Y) );
   ResMat(Ib,3) = std( Data(If,Col.Y) )./sqrt(length(If));
