...
function [tp,fp,ppv,mcc,Fscore] = roc(data);
...
fp = cumsum(~t)/sum(~t);
ppv = tp./(tp + fp);
mcc = (cumsum(t).*(sum(~t)-cumsum(~t)) - cumsum(~t).*(sum(t)-cumsum(t)))./sqrt((cumsum(t)+cumsum(~t)).*(cumsum(t)+(sum(t)-cumsum(t))).*(cumsum(~t)+cumsum(~t)).*(cumsum(~t)+(sum(t)-cumsum(t))));
Fscore = 2.*cumsum(t)./(2.*cumsum(t)+cumsum(~t)+(cumsum(~t)+(sum(t)-cumsum(t))));
